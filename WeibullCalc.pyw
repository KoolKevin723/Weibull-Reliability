...
class Example(QtGui.QWidget):
    ...
    def initUI(self):        
        ...
        #Confidence Limits
        self.CLimits = QtGui.QCheckBox(self)
        self.CLimits.move(175, 190) 
        self.CLimits.hide()
        
        self.CLimitslabel = QtGui.QLabel("Check to add Confidence Limits", self)        
        self.CLimitslabel.move(190, 190)
        self.CLimitslabel.hide()
        self.CLimits.stateChanged.connect(self.Showconfidence)
        ...
        self.CLabel.move(175,210)
        self.CLabel.hide()
        
        self.Cbox = QtGui.QSpinBox(self)
        self.Cbox.move(175, 250)
        ...
        self.CLButton = QtGui.QPushButton("Calculate Confidence Intervals",self)
        self.CLButton.setToolTip("This will take a few seconds")
        self.CLButton.move(175, 290)
        self.CLButton.setFixedWidth(170)
        self.CLButton.hide()
        self.CLButton.clicked.connect(self.Confidence)
        
        self.clabel = QtGui.QLabel(" -- ", self)
        self.clabel.setFixedWidth(100)
        self.clabel.move(175, 270 )
        self.clabel.hide()
    ...
    def Showconfidence(self):
        if self.CLimits.isChecked(): 
            self.clabel.show()
            self.Cbox.show()
            self.CLabel.show()
            self.CLButton.show()
            self.PlotButton.move(175,320)
        else:
            self.clabel.hide()
            self.Cbox.hide()
            self.CLabel.hide()
    ...
    def Plot(self):
        ...
        #Add B-Life to plot
        if self.bplot.isChecked() and self.CLimits.isChecked() == False:
            ax.plot(self.blife,log(-log(1-(self.Rbox.value()/100.0))), '+', ms=20, mew=2.5,color = "#98FB98", label = "B%.0f-Life = %.0f" % (self.Rbox.value(), self.blife) )
            plt.legend(loc='lower right', numpoints = 1)
        
        if self.bplot.isChecked() and self.CLimits.isChecked():
